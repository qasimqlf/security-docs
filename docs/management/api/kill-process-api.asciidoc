...
|`parameters.pid` |Number |The PID of the process to terminate. |Yes* (Either `parameters.pid` or `parameters.entity_id` must be provided, but not both)
|`parameters.entity_id` |String |The entity ID of the process to terminate. |Yes* (Either `parameters.pid` or `parameters.entity_id` must be provided, but not both)
|==============================================


===== Example requests

Terminates the process with entity_id `abc123` on a host with an `endpoint_id` value of `ed518850-681a-4d60-bb98-e22640cae2a8` and comments `hasta la vista`:
